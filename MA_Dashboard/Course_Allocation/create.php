...
<?php
    if(isset($_POST["enter"])){
        ...
        $LecID = $_POST["Lecture-ID"];
        $Batch = $_POST["Batch"];
        $AcademicYear = $_POST["AcademicYear"];

        require_once "../connect.php";

        // Insert into courseallocation table
        $sql = "INSERT INTO courseallocation (CourseCode, `Lecture-ID`, Batch, AcademicYear) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
        if($prepareStmt){
            mysqli_stmt_bind_param($stmt, "ssss", $CourseCode, $LecID, $Batch, $AcademicYear);
            mysqli_stmt_execute($stmt);
            
            // Get the last inserted AllocationID
            $allocationID = mysqli_insert_id($conn);

            // Create a dynamic table name based on AllocationID
            $tableName = "allocation_".$allocationID;

            // Create the new table with required columns
            ...
            if(mysqli_query($conn, $createTableSql)){
                // Insert RegNo from student table into the dynamic table
                $insertRegNoSql = "INSERT INTO $tableName (RegNo) SELECT RegNo FROM student WHERE Batch = ?";
                $stmt2 = mysqli_stmt_init($conn);
                $prepareStmt2 = mysqli_stmt_prepare($stmt2, $insertRegNoSql);
                if($prepareStmt2){
                    mysqli_stmt_bind_param($stmt2, "s", $Batch);
                    mysqli_stmt_execute($stmt2);
                    mysqli_stmt_close($stmt2);

                    $_SESSION["create"] = "Course Allocation is Done!";
                    header("Location: Course_Allocation.php");
                    exit(); // Ensure script stops executing after redirect
                } else {
                    die("Error inserting RegNo: " . mysqli_error($conn));
                }
            } else {
    ...
    <div class="card head-card p-3 mt-2">
        <div class="card-body">
            <header class="d-flex justify-content-between my-3">
                <h1><i class="bi bi-folder-plus"></i> Add Course Allocation</h1>
                <div>
                    <a href="Course_Allocation.php" class="btn btn-danger btn-lg"><i class="bi bi-arrow-left"></i> Back</a>
                </div>
            </header>
            <div class="row">   
                <div class="col-md-2">
                ...
                <form action="create.php" method="post">
                    <div class="input-group input-group-lg mb-3">
                        ...
                        <select class="form-control" id="CourseCode" name="CourseCode">
                            <?php
                            require_once "../connect.php";
                            $sql = "SELECT CourseCode FROM course ORDER BY Semester";
                    ...
                    <div class="input-group input-group-lg mb-3">
                        ...
                        <select class="form-control" id="Lecture-ID" name="Lecture-ID">
                            <?php
                            $sql = "SELECT * FROM lecturer";
                            $result = mysqli_query($conn, $sql);
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<option value='" . $row['Lecture-ID'] . "'>" . $row['Lecture-ID'] . " (" . $row['FirstName'] . " " . $row['MidName'] . " " . $row['LastName'] . ")</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-lg mb-3">
                        ...
                        <input type="text" class="form-control" name="AcademicYear" placeholder="YYYY">
                    </div>

                    <div class="input-group mb-5">
                        <input type="submit" class="btn btn-success btn-lg" name="enter" value="Save">
                    </div>
                </form>
                </div>
            </div>
