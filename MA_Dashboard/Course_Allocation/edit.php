...
<head>
  ...
  <title>Edit Course allocation</title>
  <!-- Bootstrap CSS -->
    ...
    <div class="card-body">
      <?php
          if (isset($_GET['AllocationID'])) {
              ...
              include("../connect.php") ;
              $sql = "SELECT * FROM CourseAllocation WHERE AllocationID = '$allocationID'";
              $result = mysqli_query($conn,$sql);
              $row = mysqli_fetch_array($result);
      ?>
              <header class="d-flex justify-content-between my-3">
              <h1><i class="bi bi-pencil-square"></i> Edit Course Allocation</h1>
              <div>
              ...
              <div class="col-md-8">
                <form action="process.php" method="post">
                    <div class="input-group input-group-lg mb-3">
                      <span class="input-group-text">Course Code</span>
                        <select class="form-control" id="CourseCode" name="CourseCode">
                            ...
                            $sql = "SELECT DISTINCT CourseCode FROM course ORDER BY Semester";
                            $result = mysqli_query($conn, $sql);
                            ?>
                            <option selected><?php echo $row['CourseCode']; ?></option>
                            <?php
                            while ($row1 = mysqli_fetch_assoc($result)) {
                                echo "<option value='" . $row1['CourseCode'] . "'>" . $row1['CourseCode'] . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="input-group input-group-lg mb-3">
                      <span class="input-group-text">Batch</span>
                        <select class="form-control" id="Batch" name="Batch">
                            ...
                            while ($row1 = mysqli_fetch_assoc($result)) {
                                ?>
                                    <option value="<?= $row1['Batch']; ?>" ><?= $row1['Batch']; ?></option>
                                <?php
                            }
                            ?>
                        </select>
                      ...
                      <div class="input-group input-group-lg mb-3">
                        ...
                        <select class="form-control" id="Lecture-ID" name="Lecture-ID">
                            ...
                            $result = mysqli_query($conn, $sql);
                            ?>
                            <option selected><?php echo $row['Lecture-ID']; ?></option>
                            <?php
                            while ($row1 = mysqli_fetch_assoc($result)) {
                            ?>
                                <option value="<?php echo $row1['Lecture-ID']; ?>">
                                    <?php echo $row1['Lecture-ID'] . ' (' . $row1['FirstName'] . ' ' . $row1['MidName'] . ' ' . $row1['LastName'] . ')'; ?>
                                </option>
                            <?php
                            }
                            ?>
                    ...
                    <span class="input-group-text">Academic Year</span>
                        <input type ="text" name="AcademicYear"  value="<?php echo $row["AcademicYear"];?>"  class="form-control" >
                        <input type="hidden" name="AllocationID" value="<?php echo $row["AllocationID"]; ?>">
                    </div>
                    
                    <div class="input-group mb-5">
                        <input type="submit" value="Submit" class="btn btn-success btn-lg" name ="edit"> 
                    </div>
                  </form>
                  </div>
