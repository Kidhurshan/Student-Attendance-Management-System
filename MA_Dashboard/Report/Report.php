...
$courseNames = $conn->query("SELECT DISTINCT CourseName, CourseCode FROM Course");
$batches = $conn->query("SELECT DISTINCT Batch FROM CourseAllocation");
$lecturerNames = $conn->query("SELECT DISTINCT CONCAT(LEFT(l.FirstName, 1), IF(l.MidName IS NOT NULL, CONCAT('. ', LEFT(l.MidName, 1), '. '), ' '), l.LastName) AS LecturerName FROM Lecturer l");
...
$filterCourseName = $filterBatch = $filterLecturerName = "";
$students = [];

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $filterCourseName = $_POST['CourseName'];
    $filterBatch = $_POST['Batch'];
    $filterLecturerName = $_POST['LecturerName'];
    ...
    $allocationQuery = "
        ...
        WHERE c.CourseName = ? AND a.Batch = ? AND CONCAT(LEFT(l.FirstName, 1), IF(l.MidName IS NOT NULL, CONCAT('. ', LEFT(l.MidName, 1), '. '), ' '), l.LastName) = ?
    ";

    $stmt = $conn->prepare($allocationQuery);
    $stmt->bind_param("sss", $filterCourseName, $filterBatch, $filterLecturerName);
    $stmt->execute();
        ...
        <div class="card-body">
          ...
          <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <div class="row mb-4">
              ...
              <div class="col-md-4">
                <label for="LecturerName" class="form-label"><i class="bi bi-mortarboard"></i> Lecturer:</label>
                <select name="LecturerName" id="LecturerName" class="form-select">
                  <option value="">Select the Lecturer</option>
                  <?php while($row = $lecturerNames->fetch_assoc()): ?>
