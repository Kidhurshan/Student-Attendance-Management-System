...
if(isset($_POST["enter"])){
    ...
    if($rowCount > 0){
        $_SESSION["error"] = "Course Code is invalid or Already registered";
        header("Location: Course_List.php");
        exit();
    } else {
        $sql = "INSERT INTO course (CourseCode, CourseName, Semester, Credit, LecHour) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
        if($prepareStmt){
            mysqli_stmt_bind_param($stmt, "sssss", $CourseCode, $CourseName, $Semester, $Credit, $LecHour);
            mysqli_stmt_execute($stmt);
            $_SESSION["create"] = "Course is Added";
            header("Location: Course_List.php");
            exit();
        } else {
  ...
  <div class="card head-card p-3 my-2">
    <div class="card-body">
      <header class="d-flex justify-content-between my-4">
        <h1><i class="bi bi-file-earmark-plus"></i> Add Course Registration</h1>
        <div>
        ...
        <div class="col-md-10">
          <form action="create.php" method="post">
                <div class="input-group input-group-lg mb-3">
                    ...
                    <input type="text" class="form-control" name="CourseCode" placeholder="XXXXX">
                </div>
                <div class="input-group input-group-lg mb-3">
                ...
                <div class="input-group input-group-lg mb-3">
                    ...
                    <input type="text" class="form-control" name="Semester" placeholder="1 / 2 / 3 / 4 / 5 / 6 / 7 / 8">
                </div>
                <div class="input-group input-group-lg mb-3">
                    <span class="input-group-text">Credit</span>
                    <input type="text" class="form-control" name="Credit" placeholder="1 / 2 / 3 / 4">
                </div>
                <div class="input-group input-group-lg mb-3">
                    <span class="input-group-text">Lec Hour</span>
                    <input type="text" class="form-control" name="LecHour" placeholder="1 - 50">
                </div>
                <div class="input-group my-2">
                    <input type="submit" class="btn btn-success btn-md" name="enter" value="Register">
                </div>
            </form>
        </div>
      </div>
